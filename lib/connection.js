...
export default class LibConnection {
    ...
    async conectar(state, saveCreds, version) {
        try {
            // ✅ CAMBIO PRINCIPAL: Usamos la función por defecto que es la que funciona correctamente
            this.sock = defaultWASocket({
                version: version,
                auth: state,
            ...
            // ✅ COMPROBACIÓN EXTRA ESTRICTA
            if (!this.sock || !this.sock.ev) {
                throw new Error('El objeto de conexión no se generó correctamente')
            }
            ...
            // ✅ Aquí capturamos antes de que se rompa todo
