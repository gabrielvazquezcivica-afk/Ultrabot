...
// ✅ Ahora exportado como exportación por defecto para que coincida con tu importación
export default async function connectBot () {
  ...
  const metodo = await new Promise(respuesta => {
    rl.question(
  ...
  if (metodo === '1') {
    ...
    numero = await new Promise(respuesta => {
      rl.question(
  ...
  sock.ev.on('connection.update', async update => {
    ...
    if (metodo === '2' && qr && !state.creds.registered) {
      console.log(chalk.yellow('\n📱 Escanea este código QR:\n'))
      qrcode.generate(qr, { small: true })
    }

    // 🔢 GENERAR Y MOSTRAR CÓDIGO NUMÉRICO
    if (metodo === '1' && !state.creds.registered && numero) {
      await new Promise(res => setTimeout(res, 12000))

      try {
        const codigoVinculacion = await sock.requestPairingCode(`+${numero}`)
    ...
    if (connection === 'close') {
      ...
      if (metodo === '1' && !state.creds.registered) {
