...
// 🔥 Extraer correctamente SIEMPRE (compatible con todas las versiones)
...
`));
    ...
    async iniciarProceso() {
        // ⚠️ Verificación de funciones (anti error)
        if (!useMultiFileAuthState) {
            throw new Error('Tu versión de Baileys no soporta useMultiFileAuthState. Actualiza.');
        }
        ...
        sock.ev.on('connection.update', (update) => {
            ...
            if (connection === 'open') {
                ...
                console.log(chalk.gray('→ Bot listo\n'));
                this.rl.close();
            }
